...
typedef struct so_scheduler {
	...
	unsigned int io;
	/*
	 * linked lists with ready threads, based on priority
	 */
	...
	int current_quantum;
	/*
	 * linked lists with waiting threads, based on priorities
	 */
...
/*
 * funcion tells if the current running thread needs to be replaced by the
 * candidate thread
 * + candidate: the thread that could replace the current running one
 * returns: 0 if NO and 1 if YES
 */
static int needs_to_be_replaced(so_task_t *candidate)
{
	/*
...
/*
 * the function finds the thread that will run next, blocks or unlocks threads
 * accordingly and updates the data from scheduler_so
 * + wait: 1 if the current thread also needs to be blocked or 0 otherwise
 */
static void schedule(int wait)
{
	 ...
	 */
		...
		if (scheduler.current_quantum <= 0)
			scheduler.current_quantum = scheduler.time_quantum;
		if (best_candidate)
			scheduler.ready[best_candidate->priority] =
				best_candidate_node;
...
/*
 * the function will run on each new forked thread
 * + params: data about the newly forked thread
 */
...
DECL_PREFIX tid_t so_fork(so_handler *func, unsigned int priority)
{
	...
	/*
	 * initialize thread struct (so_task)
	 */
	...
	if (pthread_create(&forked_thread->tid, NULL, start_thread,
				forked_thread))
		return INVALID_TID;
...
/*
 * move all the threas that were waiting for the io event into the ready state
 * + io: the io event that occured
 * returns: the number of threads that moved from waiting to ready
 */
static int wake_up_threads(unsigned int io)
{
